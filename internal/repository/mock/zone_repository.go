...
func (m *MockZoneRepository) FetchZone(ctx context.Context, zoneDnsName string) (*swagger.DomainZone, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "FetchZone", ctx, zoneDnsName)
	ret0, _ := ret[0].(*swagger.DomainZone)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
...
func (m *MockZoneRepositoryFactory) NewZoneRepository(config repository.Config) repository.ZoneRepository {
	m.ctrl.T.Helper()
